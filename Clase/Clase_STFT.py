...
fstft, t_stft, Zxx = stft(wav_data_c, fs=fs_audio_c, nperseg=256*4)
...
# Señal
ax1.plot(wav_data_c)
ax1.set_title("Señal La Cucaraccha")
ax1.set_ylabel("Amplitud")
    
# Subplot 2: Welch
ax2.semilogy(fwelch_c, pxx_c)
ax2.set_title("Estimación espectral por Welch")
ax2.set_ylabel("PSD [V²/Hz]")
ax2.set_xlabel("Frecuencia [Hz]")
    
# Espectrograma
pcm = ax3.pcolormesh(t_stft, fstft, np.abs(Zxx), shading='gouraud')
...
fstft, t_stft, Zxx = stft(ecg_one_lead, fs=fs_ecg, nperseg=1000)

#Estimo con welch
(fwelch_ecg, pxx_ecg) = sp.signal.welch(ecg_one_lead, fs_ecg ,nfft = N1, window = 'hamming', nperseg = N1//6, axis = 0)
...
# Subplot 2: Welch
ax2.semilogy(fwelch_ecg, pxx_ecg)
ax2.set_title("Estimación espectral por Welch")
ax2.set_ylabel("PSD [V²/Hz]")
ax2.set_xlabel("Frecuencia [Hz]")
...
#Estimo con welch
(fwelch_cuca, pxx_cuca) = sp.signal.welch(wav_data_c, fs ,nfft = N1, window = 'hamming', nperseg = N1//6, axis = 0)
